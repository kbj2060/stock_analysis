...
def modeling(BATCH_SIZE, TIME_STEPS, FEATURES_COUNT, DROPOUT_SIZE, LSTM_UNITS, LEARNING_RATE):
    ...
    model.add(LSTM(LSTM_UNITS,
                   ...
                   return_sequences=True, recurrent_dropout=DROPOUT_SIZE
                   ))
    for i in range(1):
        model.add(LSTM(LSTM_UNITS, batch_input_shape=(BATCH_SIZE, TIME_STEPS, FEATURES_COUNT), recurrent_dropout=DROPOUT_SIZE))
    model.add(Dense(1))
